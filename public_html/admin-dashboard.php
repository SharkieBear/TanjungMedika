...
// Ambil data pesanan menunggu (maksimal 5 baris)
$orders = $pdo->query("
    SELECT co.id, co.total, co.status, co.order_date, 
           CONCAT(u.first_name, ' ', u.last_name) AS customer_name 
    FROM customer_orders co 
    JOIN users u ON co.user_id = u.id 
...
// Ambil data stok habis (maksimal 5 baris)
$stock = $pdo->query("SELECT * FROM products WHERE stock < 10 LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);

// Ambil data foto profil dari session
$foto_profil = $_SESSION['pengguna']['foto_profil'] ?? 'images/profile.jpg';

// Ambil data penjualan
$today = date('Y-m-d');
$stmt = $pdo->prepare("SELECT SUM(total) as total FROM customer_orders WHERE DATE(order_date) = :today");
$stmt->execute(['today' => $today]);
$dailySales = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Penjualan mingguan
$startOfWeek = date('Y-m-d', strtotime('monday this week'));
$endOfWeek = date('Y-m-d', strtotime('sunday this week'));
$stmt = $pdo->prepare("SELECT SUM(total) as total FROM customer_orders WHERE DATE(order_date) BETWEEN :startOfWeek AND :endOfWeek");
$stmt->execute(['startOfWeek' => $startOfWeek, 'endOfWeek' => $endOfWeek]);
$weeklySales = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Penjualan bulanan
$startOfMonth = date('Y-m-01');
$endOfMonth = date('Y-m-t');
$stmt = $pdo->prepare("SELECT SUM(total) as total FROM customer_orders WHERE DATE(order_date) BETWEEN :startOfMonth AND :endOfMonth");
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                <h3 class="text-xl font-semibold text-yellow-700 mb-4">Penjualan Harian</h3>
                <p class="text-2xl font-bold text-gray-800">
                    Rp <?php echo number_format($dailySales, 0, ',', '.'); ?>
                </p>
                <p class="text-sm text-gray-500 mt-2">Tanggal: <?php echo date('d M Y'); ?></p>
            </div>

            <!-- Penjualan Mingguan -->
            <div class="bg-white p-6 rounded-lg shadow-md">
                <h3 class="text-xl font-semibold text-green-700 mb-4">Penjualan Mingguan</h3>
                <p class="text-2xl font-bold text-gray-800">
                    Rp <?php echo number_format($weeklySales, 0, ',', '.'); ?>
                </p>
                <p class="text-sm text-gray-500 mt-2">
                    Periode: <?php echo date('d M Y', strtotime('monday this week')); ?> - <?php echo date('d M Y', strtotime('sunday this week')); ?>
                </p>
            </div>

            <!-- Penjualan Bulanan -->
            <div class="bg-white p-6 rounded-lg shadow-md">
                <h3 class="text-xl font-semibold text-purple-700 mb-4">Penjualan Bulanan</h3>
                <p class="text-2xl font-bold text-gray-800">
                    Rp <?php echo number_format($monthlySales, 0, ',', '.'); ?>
                </p>
                <p class="text-sm text-gray-500 mt-2">
            ...
            <!-- Notifikasi -->
            <h2 class="text-3xl font-bold text-gray-800 mt-6">Informasi</h2>
    ...
    <?php if (!empty($orders)): ?>
        <?php foreach ($orders as $order): ?>
            <tr class="text-center">
                <td class="border border-gray-300 p-2">ORD<?= $order['id'] ?></td>
                <td class="border border-gray-300 p-2"><?= $order['customer_name'] ?></td>
                <td class="border border-gray-300 p-2">Rp<?= number_format($order['total'], 0, ',', '.') ?></td>
...
function openOrderDetail(orderId) {
    fetch(`admin-dashboard?action=get_order_detail&id=${orderId}`)
...
function updateOrderStatus(orderId) {
    ...
    window.location.href = `admin-pesanan?update_status=1&order_id=${orderId}&new_status=${newStatus}`;
}

// Fungsi untuk membuka modal edit stok dan tanggal kedaluwarsa
function openEditStock(productId) {
    fetch(`admin-dashboard?action=get_product_detail&id=${productId}`)
        .then(response => response.json())
        .then(data => {
